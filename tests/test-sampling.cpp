...
extern struct llama_sampler * llama_sampler_init_dry_testing(int32_t n_vocab, int32_t context_size, float dry_multiplier, float dry_base, int32_t dry_allowed_length, int32_t dry_penalty_last_n, const std::vector<std::vector<llama_token>>& seq_breakers);
...
static void test_penalties(
    ...
    auto * sampler = llama_sampler_init_penalties(probs.size(), last_tokens.size(), repeat_penalty, alpha_frequency, alpha_presence);
...
static void test_dry(
    ...
    auto * sampler = llama_sampler_init_dry_testing(probs.size(), 1024, dry_multiplier, dry_base, dry_allowed_length, dry_penalty_last_n, seq_breakers);
